...
class Extractor:
    ...
    def __ext4extractor(self):
        ...
        def scan_dir(root_inode, root_path=""):
            for entry_name, entry_inode_idx, entry_type in root_inode.open_dir():
                ...
                print(f'\rExtract:[{self.DIR + entry_inode_path}]', end='')
    ...
    def fix_size(self):
        ...
        with open(self.OUTPUT_IMAGE_FILE, 'rb+') as file:
            t = ext4.Volume(file)
            if orig_size > sys.maxsize or t.get_block_count > 2251799813685248 - t.block_size:
                return
            else:
    ...
    if sys.argv.__len__() == 3:
        ...
        if sys.argv.__len__() == 2:
            if not os.path.isdir("."):
                os.makedirs(".")
            Extractor().main(sys.argv[1], os.path.basename(sys.argv[1]).split('.')[0])
